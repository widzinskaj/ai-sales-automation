...
```bash
# Set test mode vars in .env, then:
...
2. The service account email has been granted **Sheets Editor** access on the spreadsheet
   (share the sheet with the service account email, same as sharing with a person).
...
- [ ] Confirm `TEST_RECIPIENT_EMAIL` is empty or absent (it is not used in production mode,
      but leaving it set to a real internal address is a safe practice in case test mode is
